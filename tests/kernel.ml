...
let open_sign_default () =
  (* Check that by default, [Sig_state.of_sign_and_solver s tc_solver_prog add_tc_inst _] opens [s] *)
  let sign = Sign.create [ "dummy" ] in
  let _ =
    Sign.add_symbol sign Term.Public Term.Defin Term.Eager false
      (Pos.none "foo") None Term.mk_Type []
  in
  let ss = Handle.Elpi_handle.(Sig_state.of_sign_and_solver sign tc_solver_prog add_tc_inst) in
  Alcotest.(check unit)
    "find_sym succeeds"
