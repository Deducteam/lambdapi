...
class TestErrorRecovery(LSPTestCase):
    ...
    def test_rapid_didChange_converges(self):
        """The final diagnostics must match the final text, however many
        changes were queued up in between."""
        uri, text, _src, _ = self.open_fixture("simple.lp")
        clean = text
        broken = text + "\nsymbol bad : Undefined;\n"
        # Flip-flop many times, no reads in between: v=2,4,… broken,
        # v=3,5,… clean; the last change (v=9) is clean.
        for v in range(2, 10):
