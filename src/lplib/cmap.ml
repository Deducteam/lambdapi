...
module Range : RangeType = struct
  ...
  let in_range pos (pos1, pos2) =
    ...
    if pos < pos1 then Before else if pos > pos2 then After else In
  ...
  let point_to_string pt =
    "Line : " ^ string_of_int pt.l ^ "; Column : " ^ string_of_int pt.c ^ "\n"
...
module MakeRangeMap (Range : RangeType) = struct
  ...
  let point_to_interval : point -> interval =
   fun pt -> Range.make_interval pt pt
...
module type SharedRangeMapType =
  RangeMapType with type point = Range.point and type interval = Range.interval
