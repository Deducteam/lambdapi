...
(** [substitution] takes the type of the goal we we want to prove and the
 * two sides of the equality proof passed to rewrite. It finds the first
 * instance o*)
let substitution : term -> term * term -> term * term = fun g_type (l, r) ->
  assert false
...
let handle_rewrite : term -> unit = fun t ->
  ...
  in
  let (ctx, t_type) = break_prod t_type in
