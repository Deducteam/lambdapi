...
(** [whnf t] computes a weak head-normal form of the term [t] wrt
   beta-reduction, user-defined rewrite rules and variables defined in the
   context [c]. *)
...
(** [eval cfg c t] evaluates the term [t] in the context [c] according to
    configuration [cfg]. *)
val eval : config -> ctxt -> term -> term
...
val tree_walk : problem -> dtree -> ctxt -> stack -> (term * stack) option
