...
(** {1 Conditions for decision trees}
    ...
    not left-linear, for example [f &x &x (s &y) ᐅ r]. In this case we need to
    ...
    term. If there is a rule of the form [f (λ x y, &Y[y]) ᐅ &Y], then we need
...
(** Clause matrices encode pattern matching problems.  The clause matrix {i C}
    can be denoted {i C = P ᐅ A} where {i P} is a {e pattern matrix} and {i A}
...
module CM = struct
  ...
  (** A clause matrix row (schematically {i c_lhs ᐅ c_rhs if cond_pool}). *)
  type clause =
    { c_lhs : term array
  ...
  let pp : ?pp_cond:bool -> t pp = fun ?(pp_cond=false) oc m ->
    let pp_lhs oc lhs =
      Format.fprintf oc "@[%a ᐅ … @]" (Array.pp Print.pp " | ") lhs
    in
    let pp_path oc l =
