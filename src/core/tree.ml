  ...
  let update_aux : int -> int -> arg list -> int VarMap.t -> clause ->
    ...
    | Patt(i, _, e) ->
        ...
        let cond_pool =
          match i with
          | Some(i) ->
              if Debug.Logger.log_enabled () then
...
let compile : match_strat -> CM.t -> tree = fun mstrat m ->
  ...
  | Condition(cond)                        ->
      if Debug.Logger.log_enabled () then log "Condition [%a]" pp_tree_cond cond;
      ...
      Cond({ok; cond; fail})
  | Check_stack                            ->
      if Debug.Logger.log_enabled () then log "Check stack";
      let left =
      ...
      Eos(left, right)
  | Specialise(swap)                       ->
      if Debug.Logger.log_enabled () then log "Swap on column %d" swap;
