...
(** [lbp t] returns the binding power of term [t] (which is 0 if [t] is not an
    operator). *)
...
(** [nud t] is the production of term [t] with {b no} left context. If [t] is
    not an operator, [nud] is the identity. Otherwise, the output is a
    production rule. *)
let rec nud : p_term Stream.t -> p_term -> p_term = fun strm t ->
  ...
  | P_Iden({elt = (_, s); pos}, _) ->
      begin match StrHtbl.find_opt una_operators s with
        | None -> t
        | Some((_, bp, _) as op) ->
            Pos.make pos (P_UnaO(op, parse bp strm))
      end
  | _ -> t
  ...
  | P_Iden({elt = (_, s); pos}, _) ->
      begin match StrHtbl.find_opt bin_operators s with
        ...
        | Some((_, assoc, bp, _) as op) ->
            ...
            Pos.make pos (P_BinO(left, op, parse bp strm))
      end
  | _ -> t

(** [parse rbp s] parses stream of tokens [s] with right binding power
    [rbp]. *)
and parse : priority -> 'a Stream.t -> p_term = fun rbp strm ->
  let rec aux left =
