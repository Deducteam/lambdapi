...
(** Problem used to record metavariables generated by unification rules. *)
let the_problem : problem Stdlib.ref = Stdlib.ref (new_problem())

(** [whnf ~rewrite c t] computes a whnf of the term [t] in context [c] using
   user-defined rewrite rules only if [~rewrite = true]. *)
let rec whnf : ?rewrite:bool -> ctxt -> term -> term =
  fun ?(rewrite=true) c t ->
  (*if !log_enabled then log_eval "whnf %a" pp_term t;*)
  let s = Stdlib.(!steps) in
  let u, stk = whnf_stk ~rewrite c t [] in
  let r = if Stdlib.(!steps) <> s then add_args u stk else unfold t in
  (*if !log_enabled then log_eval "whnf %a" pp_constr (c,t,r);*) r

(** [whnf_stk ~rewrite c t stk] computes a whnf of [t] applied to [stk] in
   context [c] using user-defined rewrite rules only if [~rewrite = true]*)
and whnf_stk : ?rewrite:bool -> ctxt -> term -> stack -> term * stack =
  fun ?(rewrite=true) c t stk ->
  (*if !log_enabled then
    log_eval "whnf_stk %a%a %a" pp_ctxt c pp_term t (D.list pp_term) stk;*)
  ...
  | (Symb s, stk) as r when rewrite ->
      ...
      | _ ->
      match tree_walk !(s.sym_dtree) c stk with
      | None -> r
...
(** [tree_walk tr c stk] tries to apply a rewrite rule by matching the stack
   [stk] against the decision tree [tr] in context [c]. The resulting state
  ...
  let rec walk tree stk cursor vars_id id_vars =
    ...
    | Leaf(rhs_subst, (act, xvars)) -> (* Apply the RHS substitution *)
        ...
        for i = env_len - xvars to env_len - 1 do
          let p = Stdlib.(!the_problem) in
          let mt = LibMeta.make p c mk_Type in
          let t = LibMeta.make p c mt in
    ...
    | Cond({ok; cond; fail})                              ->
        let next =
          ...
          | CondFV(i,xs) ->
              ...
              let b = Bindlib.bind_mvar allowed (lift (snf c vars.(i))) in
              if no_forbidden b
...
and snf : ctxt -> term -> term = fun c t ->
  let h = whnf c t in
  match h with
  | Vari(_)     -> h
  | Type        -> h
  | Kind        -> h
  | Symb(_)     -> h
  | LLet(_,t,b) -> snf c (Bindlib.subst b t)
  | Prod(a,b)   ->
      let (x,b) = Bindlib.unbind b in
      let b = snf c b in
  ...
  | Abst(a,b)   ->
      ...
      let b = Bindlib.unbox (Bindlib.bind_var x (lift b)) in
      mk_Abst(snf c a, b)
  | Appl(t,u)   -> mk_Appl(snf c t, snf c u)
  | Meta(m,ts)  -> mk_Meta(m, Array.map (snf c) ts)
...
let rec hnf : ctxt -> term -> term = fun c t ->
  match whnf c t with
  | Abst(a,t) ->
     let x,t = Bindlib.unbind t in
     mk_Abst(a, Bindlib.unbox (Bindlib.bind_var x (lift (hnf c t))))
  | t -> t
...
(** [eval cfg c t] evaluates the term [t] in the context [c] according to
    configuration [cfg]. *)
let eval : config -> ctxt -> term -> term = fun cfg c t ->
  match cfg.strategy, cfg.steps with
  | _, Some 0
  | NONE, _ -> simplify t
  | WHNF, None -> whnf c t
  | SNF, None -> snf c t
...
let tree_walk : problem -> dtree -> ctxt -> stack -> (term * stack) option =
  fun p dt c ts -> Stdlib.(the_problem := p); tree_walk dt c ts
