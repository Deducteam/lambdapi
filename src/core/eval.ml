...
and whnf_stk : ?rewrite:bool -> ctxt -> term -> stack -> term * stack =
  ...
  | Appl(f,u), stk ->
      ...
      whnf_stk c f (appl_to_tref u::stk)
  | Abst(_,f), u::stk ->
      (* Beta reduction. *)
      Stdlib.incr steps;
      whnf_stk c (Bindlib.subst f u) stk
  | LLet(_,t,u), stk ->
      (* Treat as beta-redex. *)
      (*whnf_stk c (mk_Appl(mk_Abst(a,u),t)) stk*)
      Stdlib.incr steps;
      whnf_stk c (Bindlib.subst u (appl_to_tref t)) stk
...
(** [eq_modulo c a b] tests the equality of [a] and [b] modulo rewriting and
    the unfolding of variables from the context [c] (δ-reduction). *)
