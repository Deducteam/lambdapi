...
let check_cps : Pos.popt -> Sign.t -> sym_rule list -> unit =
  fun pos sign rs ->
  check_cp_rules pos rs;
  iter_sym_rules (check_cp_subterms_eq pos) rs;
  check_cp_sign pos sign rs;
  let f path map =
    if path != Unif_rule.path && map <> StrMap.empty then
      let sign =
        try Path.Map.find path !Sign.loaded
