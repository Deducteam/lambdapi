...
line:
  | s=ID ps=param* COLON a=term DOT {
      make_pos $loc (P_symbol(None, Some Terms.Constant, make_pos $loc(s) s, ps, a))
    }
  | KW_DEF s=ID COLON a=term DOT {
      make_pos $loc (P_symbol(None, None, make_pos $loc(s) s, [], a))
    }
  | KW_INJ s=ID COLON a=term DOT {
      make_pos $loc (P_symbol(None, Some Terms.Injective, make_pos $loc(s) s, [], a))
    }
  | KW_PRV s=ID COLON a=term DOT {
      make_pos $loc (P_symbol(Some Terms.Protected, None, make_pos $loc(s) s, [], a))
    }
  | KW_DEF s=ID COLON a=term DEFEQ t=term DOT {
      make_pos $loc (P_definition(None,false, make_pos $loc(s) s, [],
                                  Some(a), t))
    }
  | KW_DEF s=ID DEFEQ t=term DOT {
      make_pos $loc (P_definition(None,false, make_pos $loc(s) s, [], None, t))
    }
  | KW_DEF s=ID ps=param+ COLON a=term DEFEQ t=term DOT {
      make_pos $loc (P_definition(None,false, make_pos $loc(s) s, ps, Some(a), t))
    }
  | KW_DEF s=ID ps=param+ DEFEQ t=term DOT {
      make_pos $loc (P_definition(None,false, make_pos $loc(s) s, ps, None, t))
    }
  | KW_THM s=ID COLON a=term DEFEQ t=term DOT {
      make_pos $loc (P_definition(None,true , make_pos $loc(s) s, [], Some(a), t))
    }
  | KW_THM s=ID ps=param+ COLON a=term DEFEQ t=term DOT {
      make_pos $loc (P_definition(None,true , make_pos $loc(s) s, ps, Some(a), t))
    }
  | rs=rule+ DOT {
