...
module Loc_t = struct
  ...
  let of_pos (p : Pos.pos) : t = Obj.magic p
  let to_pos (p : t) : Pos.pos = Obj.magic p
...
module Parsing : sig
  ...
  module Parsable = struct
    ...
    let make ?loc x = x
    let loc pa = Obj.magic 0
  end

  let parse ~token:_ ~st:_ pa = Protect.E.error (Pp_t.str "FIXME")
...
module Utils = struct
  ...
  (* Rocq lines start at 1, lsp lines start at 0 *)
...
module Init = struct
  let init () = initial_state "toplevel"
end
...
module Interp = struct
  let interp ~token:_ ~st:_ cmd = Protect.E.error (Pp_t.str "FIXME")
end
