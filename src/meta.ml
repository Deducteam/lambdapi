...
let fresh : string -> 'a map -> id = fun s m ->
...
let add_fresh : string -> 'a -> 'a map -> id * 'a map = fun s a m ->
  try
    let n, ms = StrMap.find s m in
    let k = n+1 in
