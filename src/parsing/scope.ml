...
(** [scope expo ss env mok mon ms t] turns into a term a pterm [t] in the
   ...
   already exissts a meta with name [n]. Generated metas are added to [ms]. If
...
let scope_term :
  ...
  let d = {m_term_new_metas; m_term_meta_of_key;
           m_term_meta_of_name; m_term_prv} in
  Bindlib.unbox (scope (M_Term d) ss env t)

(** [scope_term_with_params expo ss env mok mon ms t] is similar to
   [scope_term expo ss env mok mon ms t] except that [t] must be a product or
...
let scope_term_with_params :
  ...
  let scope_b cons xs u =
    Bindlib.unbox
      (scope_binder ~warn:false (M_Term d) ss cons env xs (Some u))
  in
  match t.elt with
