...
let rec scope : int -> mode -> sig_state -> env -> p_term -> tbox =
  fun d md ss env t -> scope_parsed d md ss env (Pratt.parse ss env t)
(** [scope_parsed md ss env t] turns a parser-level, Pratt-parsed term [t]
   into an actual term. *)
and scope_parsed : int -> mode -> sig_state -> env -> p_term -> tbox =
  fun d md ss env t ->
  if Logger.log_enabled () then log_scop "%a%a" D.depth d Pretty.term t;
  (* Extract the spine. *)
  ...
  let h = scope_head d md ss env p_head in
...
and scope_binder : ?warn:bool -> int -> mode -> sig_state ->
  ...
  p_term option -> tbox =
  fun ?(warn=true) d md ss cons env params_list t ->
...
and scope_head : int -> mode -> sig_state -> env -> p_term -> tbox =
  ...
  | (P_Meta({elt;pos},ts), M_Term d) ->
      ...
      let ts =
        ...
        | Some ts -> Array.map (scope (depth+1) md ss env) ts
      in
      _Meta m ts
  ...
  | (P_Patt(id,ts), M_LHS(d)) ->
      ...
      let scope_var t =
        match unfold (Bindlib.unbox (scope (depth+1) md ss env t)) with
        | Vari(x) -> x
  ...
  | (P_Abst(xs,t), _) -> scope_binder depth md ss _Abst env xs (Some(t))

  | (P_Prod(_,_), M_Patt) ->
      fatal t.pos "Dependent products are not allowed in a pattern."
  | (P_Prod(xs,b), _) -> scope_binder depth md ss _Prod env xs (Some(b))

  | (P_LLet(x,xs,a,t,u), (M_Term _|M_URHS _|M_RHS _)) ->
      let a = scope_binder (depth+1) md ss _Prod env xs a in
      let t = scope_binder (depth+1) md ss _Abst env xs (Some(t)) in
      let v = new_tvar x.elt in
      let u = scope (depth+1) md ss (Env.add v a (Some(t)) env) u in
  ...
  | (P_Wrap ({ elt = (P_Iden _ | P_Abst _); _ } as id), _) ->
      scope_head (depth+1) md ss env id
  | (P_Wrap t, _) -> scope (depth+1) md ss env t
...
let scope_term_with_params :
  ...
  fun m_term_prv ss env
      m_term_new_metas m_term_meta_of_key m_term_meta_of_name t ->
  if Logger.log_enabled () then log_scop "%a%a" D.depth 0 Pretty.term t;
  let md = M_Term {m_term_new_metas; m_term_meta_of_key;
                   m_term_meta_of_name; m_term_prv} in
