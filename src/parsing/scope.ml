...
type mode =
  | M_Term of bool
  (** Standard scoping mode for terms holding a boolean indicating whether
      private symbols are allowed. *)
  | M_Patt
...
and scope_head : mode -> sig_state -> env -> p_term -> tbox =
  ...
  | (P_Iden(qid,_), M_Term true) -> find_qid false true ss env qid
